...
// TMB FPGA and PROMs
	...
	usercode  = 0x01234567;	// Nonsense values should get over-written by jtag
	idcode    = 0x89ABCDEF;

// Select TMB FPGA Mezzanine FPGA programming JTAG chain from TMB boot register
...
//	fprintf(log_file,"main:         tdo="); for (i=0;i<reg_len; ++i) {fprintf(log_file,"%1X",tdo[i]);} fprintf(log_file,"\n");

// Interpret ID code
	tdi_to_i4(tdo,idcode,32,0);		// Deserialize

	if (chip_id==0) {				// FPGA ID code
	idcode_v = (idcode >> 28) & 0xF;
	idcode_f = (idcode >> 21) & 0x7F;
	idcode_a = (idcode >> 12) & 0x1FF;
	idcode_c = (idcode >> 1 ) & 0x7FF;}
	else {							// PROM ID code
	idcode_v = (idcode >> 28) & 0xF;
	idcode_f = (idcode >> 20) & 0xFF;
	idcode_a = (idcode >> 12) & 0xFF;
	idcode_c = (idcode >> 1 ) & 0x7FF;
	}

	ssize = "Undefined";
	if       (idcode==0x11050093) {ssize = "XC2V4000";}
	if       (idcode==0x2424C093) {ssize = "XC6VLX195T";}
	else if	 (idcode==0x05026093) {ssize = "XC18V04_AEN";}
	else if	 (idcode==0x05036093) {ssize = "XC18V04_ART";}
	else if	 (idcode==0xF5036093) {ssize = "XC18V04_BRT";}
	else if  (idcode==0x05022093) {ssize = "256K";}
	else if  (idcode==0x05023093) {ssize = "512K";}
	else if  (idcode==0x05033093) {ssize = "512K";}
	else if  (idcode==0xD5059093) {ssize = "XCF32P";}
	else						  {ssize = "Unknown";}	

	fprintf(stdout,"\tTMB %s Device=%1i IDcode  =%8.8X Ver=%1X Fam=%2.2X Arr=%3.3X Cmp=%3.3X %s\n",
	tmb_chip_type[chip_id].c_str(),chip_id,idcode,idcode_v,idcode_f,idcode_a,idcode_c,ssize.c_str());

	idcode_decode (idcode, sdevice_type, sdevice_name, sdevice_version, sdevice_size);

	fprintf(stdout,"\tTMB %s Device=%1i IDcode=%8.8X Name=%s\tVer=%s\tSize=%s\n",
	sdevice_type.c_str(), chip_id, idcode, sdevice_name.c_str(), sdevice_version.c_str(), sdevice_size.c_str());
...
// Read FPGA/PROM USERCodes (8 bit opcode)
	...
	fprintf(stdout,"\tTMB Mez  %s Device %1i USERcode=%8.8X\n",tmb_chip_type[chip_id].c_str(),chip_id,usercode);
	}	// Close for chip_id
...
// User PROM IDCodes
	...
	tdi_to_i4(&tdo[0],idcode,32,0);		// Deserialize
	...
	fprintf(stdout,"\tXilinx User PROM%1i IDcode  =%8.8X %s\n",chip_id,idcode,ssize.c_str());

	idcode_decode (idcode, sdevice_type, sdevice_name, sdevice_version, sdevice_size);

	fprintf(stdout,"\tTMB User %s Device=%1i IDcode=%8.8X Name=%s\tVer=%s\tSize=%s\n",
	sdevice_type.c_str(), chip_id, idcode, sdevice_name.c_str(), sdevice_version.c_str(), sdevice_size.c_str());
...
// User PROM USERcodes
	...
	fprintf(stdout,"\tXilinx User PROM%1i USERcode=%8.8X\n",chip_id,usercode);
	}	// close chip_id
...
// Read FPGA/PROM IDcodes (8 bit opcode)
	...
	fprintf(stdout,"\tRAT %s device %1i IDcode  =%8.8X\n",rat_chip_type[chip_id].c_str(),chip_id,idcode);
	}	// close for chip_id
...
// Read FPGA/PROM USERCodes (8 bit opcode)
	...
	fprintf(stdout,"\tRAT %s device %1i USERcode=%8.8X\n",rat_chip_type[chip_id].c_str(),chip_id,usercode);
	}	// close for chip_id
