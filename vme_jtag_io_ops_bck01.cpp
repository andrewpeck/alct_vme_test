...
// JTAG IO Operations
//
// 10/06/08 Port from fortran version
// 12/13/10 Add jtag cable detection
// 04/16/12 Add ALCT slow control chain
//------------------------------------------------------------------------------
// Headers
//------------------------------------------------------------------------------
	#include <stdio.h>
...
//------------------------------------------------------------------------------
//	Converts integer*4 word to 1 bit per byte for jtag transmission
...
//------------------------------------------------------------------------------
//	Converts tdi of 1 bit per byte into an integer*4 word
//	SPI=1 for DACs that take msb first
//------------------------------------------------------------------------------
{
	long int	i4_copy;
...
//------------------------------------------------------------------------------
// Takes jtag TAP controller from any state to RTI
//------------------------------------------------------------------------------
{
...
//
//
// 05/08/00 Changed tms_post_opcode to 2 TCKs instead of 3
// 10/27/01 Adapted for TMB2001A Xilinx User PROMs
// 06/02/04 Modified for multiple chains to support RAT and TMB
// 06/03/04 Modified for TMB2005a mezzanine chain
// 10/09/08 Convert to c++
// 06/10/11 Add Virtex-6
// 04/16/12 Add Spartan-6 and ALCT slow control PROM
//------------------------------------------------------------------------------
{
	const int	mxbits    = 128;						// Maximum # bits of tms
	const int	mxchain   = 7;							// Chain_id
	...
	int			devices_in_chain[mxchain]={2,2,2,5,3,2,3};	// Chain 0,1,2,3,4,5,6
	int			bits_per_opcode[mxchain][mxdevices]={
				...
				{10,16,16, 0, 0},		// 4 TMB  Virtex6  mez chain	FPGA, PROM, PROM
...
// Set up TMS to take TAP from RTI to Shift IR
	...
	if (ichain==0x04) chain_id = 3;						// TMB  mez  FPGA + 4 PROMs, Virtex2
	if (ichain==0x14) chain_id = 4;						// TMB  mez  FPGA + 2 PROMs, Virtex6
	if (ichain==0x01) chain_id = 5;						// ALCT slow pgm chain PROM only
	if (ichain==0x12) chain_id = 6;						// ALCT mez  FPGA + 2 PROMS, Spartan6
...
//
//
// 05/08/00 Changed tms_post_opcode to 2 bits instead of 3
// 10/26/01 Adapted for TMB2001A Xilinx User PROMs
// 06/02/04 Modified for multiple chains to support RAT and TMB
// 06/03/04 Modified for TMB2005a mezzanine chain
// 10/09/08 Convert to c++
//------------------------------------------------------------------------------
...
// Set up TMS to take TAP from RTI to Shift DR
	...
	if (ichain==0x04) chain_id = 3;						// TMB  mez FPGA + 4 PROMs, Virtex2
	if (ichain==0x14) chain_id = 4;						// TMB  mez FPGA + 2 PROMs, Virtex6
	if (ichain==0x01) chain_id = 5;						// ALCT slow pgm chain
	if (ichain==0x12) chain_id = 6;						// ALCT mez  FPGA + 2 PROMS, Spartan6
	if (chain_id==-1) pause("vme_jtag_write_ir: chain_id not implemented");
...
//------------------------------------------------------------------------------
// Returns true if jtag programming cable is detected
//------------------------------------------------------------------------------
{
// Prototypes
