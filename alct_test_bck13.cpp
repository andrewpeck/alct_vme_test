...
//	6-bit Opcodes
//
//	#	Opcode	Name		Length	Description						   Done
//------------------------------------------------------------------------------
//	1.	0x00	id_reg		40		Read Slow Control ID Register		Y
//	2.	0x01	thr_rs		1		Reset Threshold DACs				Y
//	3.	0x03	TP_VAL		?		Load Test Pulse Var Gain Amp		N
//
//	4.	0x08	DAC0		12		Load DAC0							Y
//	5.	0x09	DAC1		12		Load DAC1							Y
//	6.	0x0A	DAC2		12		Load DAC2	[not used alct384]		S
//	7.	0x0B	DAC3		12		Load DAC3	[not used alct384]		S
//
//	8.	0x10	ADC0		11		Load address and read back ADC0		Y
//	9.	0x11	ADC1		11		Load address and read back ADC1		Y
//	10.	0x12	ADC2		11		Load address and read back ADC2		Y
//	11.	0x13	ADC3		11		Load address and read back ADC3		S
//	12.	0x14	ADC4		11		Load address and read back ADC4		S
//
//	13.	0x20	w_tp_group	7		Write Test Pulse Group Enable Bits	Y 
//	14.	0x21	r_tp_group	7		Read  Test Pulse Group Enable Bits	Y
//	15.	0x22	w_tp_strip	6		Write Test Pulse Strip Enable Bits	Y
//	16.	0x23	w_tp_strip	6		Read  Test Pulse Strip Enable Bits	Y
//	17.	0x24	w_standby	42		Write FE Standby Bits				Y		
//	18.	0x25	r_standby	42		Read  FE Standby Bits				Y
//	19.	0x26	W_VAR_PD	?		Write Var Gain Amp Power Down		N
//	28.	0x27	R_VAR_PD	?		Read  Var Gain Amp Power Down		N
//
//  Others		BYPASS		?		Standard JTAG Instruction			Y
//
//------------------------------------------------------------------------------
//	Headers
//------------------------------------------------------------------------------
...
sc_submenu:
	...
	printf("\n");
	printf("\tOpcode Register    Function\n");
	printf("\n");
	printf("\t0x00:  id_reg      Read Slow Control Chip ID\n");
	printf("\t0x01:  thr_rs      Reset Threshold DACs\n");
	printf("\t0x03:  TP_VAL      Load Test Pulse Var Gain Amp\n");
	printf("\n");
	printf("\t0x08:  DAC0        Load DAC0\n");
	printf("\t0x09:  DAC1        Load DAC1\n");
	printf("\t0x0A:  DAC2        Load DAC2 [not on alct384]\n");
	printf("\t0x0B:  DAC3        Load DAC3 [not on alct384]\\n");
	printf("\n");
	printf("\t0x10:  ADC0        Load address and read back ADC0\n");
	printf("\t0x11:  ADC1        Load address and read back ADC1\n");
	printf("\t0x12:  ADC2        Load address and read back ADC2\n");
	printf("\t0x13:  ADC3        Load address and read back ADC3 [not on alct384]\n");
	printf("\t0x14:  ADC4        Load address and read back ADC4 [not on alct384]\n");
	printf("\n");
	printf("\t0x20:  w_tp_group  Write Test Pulse Group Enable Bits\n");
	printf("\t0x21:  w_tp_group  Read  Test Pulse Group Enable Bits\n");
	printf("\t0x22:  w_tp_strip  Write Test Pulse Strip Enable Bits\n");
	printf("\t0x23:  r_tp_strip  Read  Test Pulse Strip Enable Bits\n");
	printf("\t0x24:  w_standby   Write FE Standby Bits\n");		
	printf("\t0x25:  r_standby   Read  FE Standby Bits\n");
	printf("\t0x26:  W_VAR_PD    Write Var Gain Amp Power Down\n");
	printf("\t0x27:  R_VAR_PD    Read  Var Gain Amp Power Down\n");
	printf("\n");
...
//------------------------------------------------------------------------------
//	Opcode 0x01 Reset Threshold DACs
//------------------------------------------------------------------------------
sc_opcode_01:
...
// Write threshold DAC reset register
	opcode  = 0x01;											// Opcode
	reg_len = 1;											// Register length

	for (i=0; i<reg_len; ++i) tdi[i]=1;						// Reset thresholds

	vme_jtag_write_ir(adr,ichain,chip_id,opcode);			// Set opcode
	vme_jtag_write_dr(adr,ichain,chip_id,tdi,tdo,reg_len);	// Write tdi, read tdo

	for (i=0; i<reg_len; ++i) tdi[i]=0;						// Un-reset thresholds
...
// Get data to write
	...
	inquire("\tWrite DAC0 addr 0-11  %3i", 0,  11, 10, dac_adr );
	inquire("\tWrite DAC0 data 0-255 %3i",-1, 255, 10, dac_data);
...
// Get data to write
	...
	printf ("\tFor ramp mode set data=-1\n");
	inquire("\tWrite DAC1 addr 0-11  %3i", 0,  11, 10, dac_adr );
	inquire("\tWrite DAC1 data 0-255 %3i",-1, 255, 10, dac_data);
...
// Check for key press in ramp mode
	if (dac_ramp)
	{
	if (!_kbhit()) goto ramp1;								// Check for keyboard hit
	ckey = _getch();	
	}
...
// Construct 11-clock ADC bit string {D3,D2,D1,D0,7*0) D3 is first bit to send
	for (adc_adr=0; adc_adr<=15; ++adc_adr)					// 11 channels + 3 internal channels (0-13) + 1 to read last ch
	{
//!	adc_adr=0xB; // half scale

	adc_word = ((adc_adr%14) << 7);							// adr 0 reads junk, adr 1-14 reads ch 0-13

// Write new ADC channel address, read previous channel data Slow Mode 5 with 16 clocks and /CS control
	opcode  = 0x10+iadc;									// Opcode
	reg_len = 11;											// Register length
...
// Store previous channel
//!	tdi_to_i4(&tdo[0],i4,10,1);								// SPI=1 ADCs that take MSB first, first 10 bits are data , ignore bit 11
//	tdi_to_i4(&tdo[1],i4,10,1);								// SPI=1 ADCs that take MSB first, first 10 bits are data , ignore bit 11
	tdi_to_i4(&tdo[0],i4,11,1);								// SPI=1 ADCs that take MSB first, first 10 bits are data , ignore bit 11
...
// Display
	...
	for (adc_ch=0; adc_ch<=0xD; ++ adc_ch)
	{
	if (adc_ch==0xA) vadc_offset = 0.5;		// Tsensor is 0.750V at 25C
...
//------------------------------------------------------------------------------
//	Opcode 0x13 Read ADC3	No ADC device on ALCT384 boards
//------------------------------------------------------------------------------
sc_opcode_13:
...
//------------------------------------------------------------------------------
//	Opcode 0x14 Read ADC4	No ADC device on ALCT384 boards
//------------------------------------------------------------------------------
sc_opcode_14:
...
// Write Test Pulse Group register
	...
	i4_to_tdi(i4=sc_w_tp_group ,&tdi[0], reg_len, 0);		// Convert integer to bit array
...
// Read Test Pulse Group register
	...
	tdi_to_i4(&tdo[0],sc_r_tp_group,reg_len,0);				// w_tp_group[31:0]
...
// Write Test Pulse Strip register
	...
	i4_to_tdi(i4=sc_w_tp_strip ,&tdi[0], reg_len, 0);		// Convert integer to bit array
...
// Read Test Pulse Group register
	...
	tdi_to_i4(&tdo[ 0],sc_r_tp_strip,reg_len,0);			// w_tp_strip[31:0]
...
// Get data to write
	...
	sc_w_standby[0] = i64 & 0xFFFFFFFF;
	sc_w_standby[1] = i64 >> 32;
...
// Write Standby register
	...
	i4_to_tdi(i4=sc_w_standby[0],&tdi[ 0], 32, 0);			// Convert integer to bit array
	i4_to_tdi(i4=sc_w_standby[1],&tdi[32], 10, 0);
...
// Read standby register
	...
	tdi_to_i4(&tdo[ 0],sc_r_standby[0],32,0);				// r_standby[31:0]
	tdi_to_i4(&tdo[32],sc_r_standby[1],10,0);				// r_standby[41:32]

	printf("\tALCT Slow Control Opcode 0x25 r_standby[41:0] = 0x%3.3X%8.8X\n",sc_r_standby[1],sc_r_standby[0]);
	pause("<cr>");
