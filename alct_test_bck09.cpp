...
// Display menu
	...
	printf("\t8:   All tests\n");
	printf("\t9:   Automatic ADB increment\n");
	...
	itest=abs(ifunc);
	if (itest==0) goto L1400_t0;
	if (itest==1) goto L1400_t1;
	if (itest==2) goto L1400_t2;
	if (itest==3) goto L1400_t3;
	if (itest==4) goto L1400_t4;
	if (itest==5) goto L1400_t5;
	if (itest==6) goto L1400_t6;
	if (itest==7) goto L1400_t7;
//	if (itest==8) goto L1400_t8;
//	if (itest==9) goto L1400_t9;
	goto L1400_menu;
...
L1400_t1:
	...
	npatterns = 1;
	goto sc_pattern_loop;

// Loop over all tests
//	for (itest=2;    itest  <=7;         ++itest   )

// Other data tests
	switch (itest)
	{
	case 1:
	case 2:	npatterns = 16;	scsi_wr_data = 0x0000;	goto sc_pattern_loop;	// Running '1'
	case 3:	npatterns = 16;	scsi_wr_data = 0x0000;	goto sc_pattern_loop;	// Running '0'
	case 4:	npatterns = 16;	scsi_wr_data = 0x0000;	goto sc_pattern_loop;	// Filling by '1'
	case 5:	npatterns = 16;	scsi_wr_data = 0xFFFF;	goto sc_pattern_loop;	// Filling by '0'
	case 6:	npatterns = 8;	scsi_wr_data = 0x0000;	goto sc_pattern_loop;	// Shifting '5's and 'A's
	case 7:	npatterns = 16;	scsi_wr_data = 0x0000;	goto sc_pattern_loop;	// Random data
	}

L1400_t2:	npatterns = 16;	scsi_wr_data = 0x0000;	goto sc_pattern_loop;	// Running '1'
L1400_t3:   npatterns = 16;	scsi_wr_data = 0x0000;	goto sc_pattern_loop;	// Running '0'
L1400_t4:   npatterns = 16;	scsi_wr_data = 0x0000;	goto sc_pattern_loop;	// Filling by '1'
L1400_t5:   npatterns = 16;	scsi_wr_data = 0xFFFF;	goto sc_pattern_loop;	// Filling by '0'
L1400_t6:   npatterns = 8;	scsi_wr_data = 0x0000;	goto sc_pattern_loop;	// Shifting '5's and 'A's
L1400_t7:   npatterns = 16;	scsi_wr_data = 0x0000;	goto sc_pattern_loop;	// Random data

// Loop over patterns and passes
sc_pattern_loop:
...
// Next pattern
	if (itest==1) scsi_wr_data =  scsi_wr_data;									// Custom data
	if (itest==2) scsi_wr_data =  (0x1 << ipattern);							// Running '1'
	if (itest==3) scsi_wr_data = ~(0x1 << ipattern) & 0xFFFF;					// Running '0'
	if (itest==4) scsi_wr_data =  (scsi_wr_data |  (0x1 << ipattern));			// Filling by '1'
	if (itest==5) scsi_wr_data =  (scsi_wr_data & ~(0x1 << ipattern)) & 0xFFFF;	// Filling by '0's
	if (itest==6) scsi_wr_data =  (scsi_wr_data |  (0x5 << ipattern*2));		// Shifting '5's and 'A's
	if (itest==7) scsi_wr_data =  (rand() | (rand()<<15)) & 0xFFFF;				// Random data (rand returns 0-0x7FFF)
...
// Read back SCSI data
	...
	printf("\tSCSI tx data readback  %4.4X\n",scsi_rd_data);
...
// Read ADB hit list, only the selected ADB should have non-zero bits
	...
	if       (adb_hit==(1<<adb_wr_ch))          sok="OK";	// ADB with hits should match selected ADB
	else if ((adb_hit==0) && (scsi_wr_data==0)) sok="OK";	// Unless the tx data is 0
	else                                        sok="ERR";	// Otherwise fail

	printf("\tADB  connectors hit    %6.6X %s\n",adb_hit,sok.c_str());
