...
rampv:
	...
	i4_to_tdi(i4=vga_data ,&tdi[0], reg_len, 1);			// SPI=1 for DACs that take MSB first
...
// Ramp mode vga gain step
	...
	{
//	vga_data = istep*10;
//	if (vga_data > 511) {istep = 0; vga_data=0;}	// THIS IS WRONG BUT IT WORKS, WHY?
//	istep++;
	...
	istep++;
	istep = istep%9;
//!	vga_data = (vga_data << 1);	// THIS IS WRONG BUT WORKS, WHY?...WHY, BECAUSE YOU NEED TO PACK 8 bits into 9
	}
...
//!	printf("\tWriting VGA DAC  vga_data        = %4i\n",vga_data);
	...
	opcode  = 0x3;
	reg_len = 9;	// THIS IS WRONG BUT OLD SLOW CONTROL REQUIRES 9 bits instead of 8

	i4_to_tdi(i4=(vga_data<<1) ,&tdi[0], reg_len, 1);		// SPI=1 for DACs that take MSB first
